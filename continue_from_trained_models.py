#!/usr/bin/env python3
"""
Continue RL Training from Your Trained Models
Integrates the fixed RL system with your existing trained models

Based on your successful training:
- IL Model: results/debug_rl_2025-06-17_17-59-26/logs/checkpoints/autoregressive_il_best_epoch_1.pt (48.3% mAP)
- World Model: results/debug_rl_2025-06-17_17-59-26/logs/checkpoints/world_model_best_epoch_2.pt (State Loss: 0.1421)
"""
...
class FixedSimplifiedExpertMatchingEnv(gym.Env):
    ...
    def _calculate_simplified_reward(self, action):
    ...
    def _record_episode_end(self, info):
...
class ExpertMatchingCallback(BaseCallback):
    ...
    def __init__(self, eval_env, expert_data, eval_freq=500):
        super().__init__()
    ...
    def _process_action_prediction(self, action_pred):
...
def load_trained_models(results_dir, logger, device='cuda'):
    """Load the trained models from your previous run."""
    
    logger.info("📂 Loading trained models from previous run...")
    
    models = {}
    
    # Load IL model (epoch 1 - best performance: 48.3% mAP)
    ...
    # Load World Model (epoch 2 - best performance: State Loss 0.1421)
...
def train_fixed_simplified_rl(world_model, train_data, config, logger, timesteps=30000):
    ...
    # Save model
    save_dir = Path("results/fixed_rl_training_continued")
...
def evaluate_rl_model_mAP(rl_model, test_data, logger):
...
def main():
    ...
    print("🔧 CONTINUING RL TRAINING FROM YOUR TRAINED MODELS")
    print("=" * 70)
    print("🎯 Loading your successful training:")
    ...
    results_dir_new = f"results/fixed_rl_continue_{timestamp}"
    Path(results_dir_new).mkdir(parents=True, exist_ok=True)
    logger = SimpleLogger(log_dir=results_dir_new, name="FixedRL_Continue")
    ...
    try:
        ...
        # Step 2: Load your trained models
        print("\n📂 Step 2: Loading your trained models...")
        models = load_trained_models(results_dir, logger, DEVICE)
        ...
        import json
        with open(f"{results_dir_new}/fixed_rl_results.json", 'w') as f:
            json.dump(results_summary, f, indent=2, default=str)
        
        print(f"\n🎉 FIXED RL TRAINING COMPLETED!")
        print(f"📁 Results saved to: {results_dir_new}")
