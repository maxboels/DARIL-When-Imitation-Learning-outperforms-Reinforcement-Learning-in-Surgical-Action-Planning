...
class SurgicalActionVisualizer:
        ...
        for frame in range(start_frame, end_frame + 1, tick_interval):
            if frame <= end_frame:
                ...
                if use_time_format:
                    ...
                    # Just show frame number
                    tick_labels.append(f"{frame}")
        
        ax.set_xticks(tick_positions)
        ax.set_xticklabels(tick_labels, rotation=45 if use_time_format else 0)
    ...
    def _add_color_legend(self, ax, legend_type="recognition"):
        ...
        if legend_type == "recognition":
            ...
            legend_elements = [
                plt.Rectangle((0, 0), 1, 1, facecolor=self.fn_color, alpha=0.8, label='GT Future'),
                plt.Rectangle((0, 0), 1, 1, facecolor=self.tp_color, alpha=0.8, label='Correct Pred'),
                plt.Rectangle((0, 0), 1, 1, facecolor=self.fp_color, alpha=0.8, label='False Pred')
            ]
    ...
    def _plot_recognition_panel(self, ax, recognition_gt, recognition_pred,
        ...
        # Get data for selected time window and actions
        gt_data = recognition_gt[start_frame:end_frame][:, selected_actions]
        pred_data = recognition_pred[start_frame:end_frame][:, selected_actions]
        ...
        im = ax.imshow(classification.T, aspect='auto', cmap=self.classification_cmap, 
        ...
        ax.set_title('Recognition Performance', fontweight='bold')
        ax.set_ylabel('Action Classes')
        
        # Format time axis
        self._format_time_axis(ax, start_frame, end_frame, use_time_format)
    ...
    def _plot_planning_panel(self, ax, planning_gt, planning_pred,
        ...
        for i, frame in enumerate(range(start_frame, end_frame)):
            if frame >= center_frame and frame < len(planning_pred):
                # For planning, we compare prediction at center_frame with future ground truth
                horizon_steps = min(rollout_horizon, planning_pred.shape[1], time_range - i)
                
                for h in range(horizon_steps):
                    future_frame_idx = i + h + 1
                    if future_frame_idx < time_range:
                        future_frame = start_frame + future_frame_idx
                        
                        if future_frame < len(planning_gt) and h < planning_gt.shape[1]:
                            for j, action in enumerate(selected_actions):
                                # Ground truth for this future time
                                gt_val = planning_gt[future_frame, h, action] if future_frame < len(planning_gt) else 0
                                # Prediction made at center_frame for this future time
                                pred_val = planning_pred[center_frame, h + (frame - center_frame), action] if h + (frame - center_frame) < planning_pred.shape[1] else 0
        ...
        ax.set_title('Planning Performance', fontweight='bold')
        ax.set_ylabel('Action Classes')
    ...
    def _plot_combined_panel(self, ax, recognition_gt, recognition_pred,
                           planning_gt, planning_pred, start_frame, end_frame,
                           selected_actions, center_frame, rollout_horizon, threshold, use_time_format):
        """Plot combined recognition and planning view with white background."""
        ...
        # Overlay planning predictions from center frame
        if center_frame < len(planning_pred):
            for h in range(min(rollout_horizon, planning_pred.shape[1])):
                future_frame_idx = (center_frame + h + 1) - start_frame
                if 0 <= future_frame_idx < time_range:
                    future_frame = center_frame + h + 1
                    
                    for j, action in enumerate(selected_actions):
                        # Get planning prediction
                        pred_val = planning_pred[center_frame, h, action]
                        pred_binary = pred_val > threshold
                        
                        # Get ground truth for that future time
                        gt_binary = False
                        if future_frame < len(recognition_gt):
                            gt_val = recognition_gt[future_frame, action]
                            gt_binary = gt_val > threshold
                        
                        # Only overlay if there's a planning prediction
                        if pred_binary:
                            if gt_binary:
        ...
        ax.set_title('Recognition & Planning Performance', fontweight='bold')
        ax.set_ylabel('Action Classes')
...
Time Window: {end_frame - start_frame} frames
