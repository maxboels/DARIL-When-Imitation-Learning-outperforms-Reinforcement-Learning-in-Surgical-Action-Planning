...
class SurgicalRewardFunction:
    ...
    Improved version with correct phase progress tracking.
    """
    ...
    def __init__(self, 
        ...
        # Track phase start frames - critical for progress calculation
        self.phase_start_frames = {}
        self.last_frame_in_phase = {}
        
        # To store the known phase durations from the current procedure
        self.current_phase_durations = {}
    ...
    def calculate_reward(self, frame_data, is_terminal=False, phase_segments=None):
        ...
        Args:
            ...
            phase_segments: Optional DataFrame with phase segment information
                            (start_frame, end_frame, phase_id)
        ...
        phase_changed = self.current_phase is not None and phase_id != self.current_phase
        
        if phase_changed:
            # Set the progress of the completed phase to 1.0
            if self.current_phase is not None:
                self.phase_progress[self.current_phase] = 1.0
                
                # Store the phase duration for future reference
                if self.current_phase in self.phase_start_frames:
                    self.current_phase_durations[self.current_phase] = frame_id - self.phase_start_frames[self.current_phase]
            
            # Check if this is a valid phase transition (moving forward)
            if (self.current_phase in self.expected_phase_sequence and 
                phase_id in self.expected_phase_sequence):
            ...
            # Reset progress for the new phase and track start frame
            if phase_id is not None:
                self.phase_progress[phase_id] = 0.0
                self.phase_start_frames[phase_id] = frame_id
        
        # If this is the first frame of any phase or a new phase
        if self.current_phase is None or phase_changed:
            self.current_phase = phase_id
            if phase_id is not None and phase_id not in self.phase_start_frames:
                self.phase_start_frames[phase_id] = frame_id
        ...
        if phase_id is not None:
            # Get the expected end frame for this phase
            phase_end_frame = None
            
            # If we have phase segment information, use that
            if phase_segments is not None:
                phase_segment = phase_segments[phase_segments['phase_id'] == phase_id]
                if not phase_segment.empty:
                    phase_end_frame = phase_segment['end_frame'].iloc[0]
            
            # If we don't have segment info, use educated guess based on previous durations
            if phase_end_frame is None:
                # If we've seen this phase before in this video
                if phase_id in self.current_phase_durations:
                    phase_duration = self.current_phase_durations[phase_id]
                else:
                    # Use historical or default duration
                    phase_duration = self.phase_duration_stats.get(phase_id, {}).get('mean', self.default_phase_duration)
                
                # Calculate expected end frame
                phase_end_frame = self.phase_start_frames.get(phase_id, frame_id) + phase_duration
            
            # Update the last known frame for this phase
            self.last_frame_in_phase[phase_id] = frame_id
            
            # Calculate progress based on position within phase
            start_frame = self.phase_start_frames.get(phase_id, frame_id)
            frames_elapsed = frame_id - start_frame
            total_frames = phase_end_frame - start_frame
            
            # Avoid division by zero
            if total_frames > 0:
                progress = min(frames_elapsed / total_frames, 1.0)
            else:
                progress = 0.0
            
            # Update progress (never decrease progress)
