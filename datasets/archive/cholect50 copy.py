...
try:
    from reward_visualization_tool import analyze_rewards_during_loading
...
def load_cholect50_data(cfg, logger, split='train', max_videos=None, test_on_train=False):
    ...
    if analyze_rewards and REWARD_ANALYZER_AVAILABLE and data:
        ...
        try:
            analysis_dir = f"reward_analysis_{original_split}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
