...
from gymnasium.spaces import MultiDiscrete, Discrete, Dict as SpaceDict
...
class WorldModelSimulationEnv(gym.Env):
    ...
    def __init__(self, world_model, video_data: List[Dict], config: Dict, 
        ...
        self.reward_weights = {
            ...
            'world_model_rewards': 0.1,  # Use world model predictions (lower weight)
        ...
        self.logger.info("🌍 World Model Simulation Environment initialized")
        self.logger.info(f"🔧 Action space: {self.action_space}")
        self.logger.info(f"📊 Max episode steps: {self.max_episode_steps}")
        self.logger.info(f"🎯 Focus: Expert demonstration matching + world model simulation")
