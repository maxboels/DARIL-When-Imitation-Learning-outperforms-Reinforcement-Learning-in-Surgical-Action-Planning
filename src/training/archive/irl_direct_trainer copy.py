...
class DirectIRLSystem(nn.Module):
    ...
    def predict_with_irl(self, il_model, state: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            il_input = state.unsqueeze(0)
            if len(state.shape) == 1:
                il_input = il_input.unsqueeze(0)
...
class DirectIRLTrainer:
    ...
    def train_direct_irl(self, train_data: List[Dict], num_iterations: int = 100):
        ...
        for iteration in tqdm(range(num_iterations), desc="Training IRL Reward Function"):
    ...
    def _train_policy_adjustment(self, expert_states: torch.Tensor, expert_actions: torch.Tensor, num_epochs: int = 30):
        ...
        for epoch in range(num_epochs):
            ...
            for i in tqdm(range(0, num_samples, batch_size), desc=f"Training Policy Adjustment Epoch {epoch+1}/{num_epochs}"):
    ...
    def _get_il_prediction(self, state: torch.Tensor) -> torch.Tensor:
        ...
        try:
            ...
            if hasattr(self.il_model, 'predict_next_action'):
                return self.il_model.predict_next_action(il_input).squeeze()
                ...
                raise ValueError("IL model interface not supported")
        except Exception as e:
            self.logger.warning(f"IL prediction failed: {e}, using random prediction")
            return torch.rand(100).to(self.device)
