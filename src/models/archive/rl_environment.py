...
# ===================================================================
# File: rl_environment_fixed.py
# Fixed RL environment with proper tensor handling
# ===================================================================
...
class SurgicalWorldModelEnv(gym.Env):
    """
    Fixed gym environment with proper tensor dimension handling
    """
    ...
    def _prepare_action_tensor(self, action):
        """
        Properly prepare action tensor with correct dimensions
        
        Args:
            action: numpy array of shape [num_action_classes] or similar
            
        Returns:
            action_sequence: torch tensor of shape [1, context_length, num_action_classes]
        """
        # Ensure action is a numpy array and flatten it to 1D
        if isinstance(action, torch.Tensor):
            action = action.cpu().numpy()
        
        action = np.array(action).flatten()
        
        # Ensure it has the right number of elements
        expected_size = self.model.num_action_classes
        if len(action) != expected_size:
            print(f"Warning: Action size {len(action)} doesn't match expected {expected_size}")
            # Pad or truncate as needed
            if len(action) < expected_size:
                action = np.pad(action, (0, expected_size - len(action)))
            else:
        ...
        # Ensure it's exactly [num_action_classes]
        action_tensor = action_tensor.view(self.model.num_action_classes)
        
        # Add batch and sequence dimensions: [1, 1, num_action_classes]
        action_tensor = action_tensor.unsqueeze(0).unsqueeze(0)
        
        # Expand to context length: [1, context_length, num_action_classes]
        action_sequence = action_tensor.expand(1, self.context_length, -1)
        
        return action_sequence
    
    def step(self, action):
        """Enhanced step function with fixed tensor handling"""
        
        # Debug: Print action info
        # if self.step_count < 3:  # Only for first few steps
            # print(f"Step {self.step_count}: action type={type(action)}, shape={getattr(action, 'shape', 'no shape')}")
        
        # Prepare action tensor with proper dimensions
        try:
            action_sequence = self._prepare_action_tensor(action)
        except Exception as e:
            print(f"Error preparing action tensor: {e}")
        ...
        with torch.no_grad():
            ...
            output = self.model(
                ...
                eval_mode='basic'  # Inference mode
            )
            ...
            for reward_type, weight in self.reward_weights.items():
                ...
                if loss_key in output:
                    ...
                    if output[loss_key].dim() > 1:
                        reward_value = -output[loss_key][:, -1].mean().item()
                    else:
                        reward_value = -output[loss_key].item()
