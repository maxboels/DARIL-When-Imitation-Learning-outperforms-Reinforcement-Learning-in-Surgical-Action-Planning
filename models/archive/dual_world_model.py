...
class EnhancedDualWorldModel(DualWorldModel):  # Inherit from your existing model
    """
    Enhanced version of DualWorldModel with additional capabilities:
    1. Clinical outcome prediction heads
    2. Uncertainty estimation for predictions
    3. Enhanced architecture for improved performance
    """
    def __init__(self, 
                 enable_outcome_prediction: bool = True,
                 enable_uncertainty_estimation: bool = True,
                 outcome_types: List[str] = None,
                 uncertainty_method: str = 'ensemble',
                 ensemble_size: int = 5,
                 dropout_samples: int = 10,
                 **kwargs):
        """
        Initialize the enhanced dual world model.
        
        Args:
            enable_outcome_prediction: Enable clinical outcome prediction
            enable_uncertainty_estimation: Enable uncertainty estimation
            outcome_types: Types of clinical outcomes to predict
            uncertainty_method: Method for uncertainty estimation ('ensemble', 'dropout', or 'evidential')
            ensemble_size: Number of ensemble models for uncertainty estimation
            dropout_samples: Number of dropout samples for MC dropout
            **kwargs: Arguments passed to parent class
        """
        super().__init__(**kwargs)
        
        self.enable_outcome_prediction = enable_outcome_prediction
        self.enable_uncertainty_estimation = enable_uncertainty_estimation
        self.uncertainty_method = uncertainty_method
        self.ensemble_size = ensemble_size
        self.dropout_samples = dropout_samples
        
        # Default outcome types if not specified
        self.outcome_types = outcome_types or [
            'procedure_success',
            'complication_risk',
            'efficiency_score',
        ...
        # Add new components
        if self.enable_outcome_prediction:
            self._add_outcome_prediction_heads()
        
        if self.enable_uncertainty_estimation:
            self._add_uncertainty_estimation()
    
    def _add_outcome_prediction_heads(self):
        """Add prediction heads for clinical outcomes."""
        # Create a separate prediction head for each outcome type
        self.outcome_heads = nn.ModuleDict()
        
        for outcome in self.outcome_types:
            self.outcome_heads[outcome] = nn.Sequential(
                nn.Linear(self.hidden_dim, self.hidden_dim),
                nn.ReLU(),
                nn.Dropout(self.dropout),
    ...
    def _add_uncertainty_estimation(self):
        """Add components for uncertainty estimation."""
        if self.uncertainty_method == 'ensemble':
            # For ensemble method, we'll use multiple heads
            self.ensemble_heads = nn.ModuleList()
            for _ in range(self.ensemble_size):
                # Each ensemble member predicts states
                head = nn.Sequential(
                    nn.Linear(self.hidden_dim, self.hidden_dim),
                    nn.ReLU(),
                    nn.Dropout(self.dropout),
            ...
            # Evidential regression outputs distribution parameters
            self.evidential_head = nn.Sequential(
                nn.Linear(self.hidden_dim, self.hidden_dim),
                nn.ReLU(),
                nn.Dropout(self.dropout),
                nn.Linear(self.hidden_dim, self.embedding_dim * 4)  # mu, v, alpha, beta
            )
            
        # Extra dropout layer for MC dropout method
        self.uncertainty_dropout = nn.Dropout(0.2)  # Higher dropout for uncertainty
    
    def forward(self, *args, **kwargs):
        """
        Extended forward pass with outcome prediction and uncertainty estimation.
        Calls parent class forward first, then adds new functionality.
        """
        # Get outputs from parent class
        outputs = super().forward(*args, **kwargs)
        
        # Add outcome predictions if enabled
        if self.enable_outcome_prediction and hasattr(self, 'outcome_heads'):
            hidden_states = outputs['hidden_states']
            
            for outcome in self.outcome_types:
                outputs[f'outcome_{outcome}'] = self.outcome_heads[outcome](hidden_states)
        
        # Add uncertainty estimation if enabled
        if self.enable_uncertainty_estimation:
            hidden_states = outputs['hidden_states']
            
            if self.uncertainty_method == 'ensemble' and hasattr(self, 'ensemble_heads'):
                ...
                # Stack predictions [ensemble_size, batch, seq_len, embedding_dim]
                ensemble_stack = torch.stack(ensemble_predictions, dim=0)
                ...
                outputs['state_pred_mean'] = ensemble_mean
                outputs['state_pred_variance'] = ensemble_var
                
            elif self.uncertainty_method == 'evidential' and hasattr(self, 'evidential_head'):
                # Evidential regression
                params = self.evidential_head(hidden_states)
                
                # Split into distributional parameters
                embedding_dim = self.embedding_dim
                mu, v, alpha, beta = torch.split(params, embedding_dim, dim=-1)
                ...
                # Store parameters and calculate uncertainty
                outputs['state_pred_mean'] = mu
                outputs['state_pred_variance'] = beta / (alpha - 1)
                outputs['evidential_params'] = {'mu': mu, 'v': v, 'alpha': alpha, 'beta': beta}
    ...
    def estimate_uncertainty(self, 
        ...
        Args:
            current_states: Current states [batch_size, seq_len, embedding_dim]
            actions: Actions to condition on [batch_size, seq_len, num_action_classes]
            n_samples: Number of samples for MC dropout (defaults to self.dropout_samples)
            
        Returns:
            Dictionary with mean predictions and uncertainty measures
        """
        if not self.enable_uncertainty_estimation:
            raise ValueError("Uncertainty estimation is not enabled")
        
        if n_samples is None:
            n_samples = self.dropout_samples
        ...
        with torch.no_grad():
            if self.uncertainty_method == 'dropout':
                ...
                for _ in range(n_samples):
                    ...
                    samples.append(outputs['state_pred'])
                
                # Stack samples [n_samples, batch, seq_len, embedding_dim]
                stacked_samples = torch.stack(samples, dim=0)
                
                # Calculate statistics
                mean_prediction = torch.mean(stacked_samples, dim=0)
                variance = torch.var(stacked_samples, dim=0)
                ...
                # Use the ensemble heads directly
                outputs = self.forward(
                    current_states=current_states,
                    actions=actions
                )
                ...
                return {
                    'mean_prediction': outputs['state_pred_mean'],
                    'variance': outputs['state_pred_variance']
                }
            
            elif self.uncertainty_method == 'evidential':
                # Use evidential regression parameters
                outputs = self.forward(
                    current_states=current_states,
                    actions=actions
                )
                ...
                return {
                    'mean_prediction': outputs['state_pred_mean'],
                    'variance': outputs['state_pred_variance'],
                    'evidential_params': outputs['evidential_params']
                }
    
    def predict_outcomes(self, 
                       states: torch.Tensor,
                       return_confidence: bool = False) -> Dict[str, torch.Tensor]:
        """
        Predict clinical outcomes from states.
        
        Args:
            states: State sequence [batch_size, seq_len, embedding_dim]
            return_confidence: Whether to return confidence intervals
            
        Returns:
            Dictionary of outcome predictions
        """
        if not self.enable_outcome_prediction:
            raise ValueError("Outcome prediction is not enabled")
        
        self.eval()
        
        with torch.no_grad():
            # First get hidden states
            outputs = self.forward(
            ...
            outcomes = {}
            for outcome in self.outcome_types:
                prediction = self.outcome_heads[outcome](hidden_states)
                outcomes[outcome] = prediction
            
            result = {'outcome_predictions': outcomes}
            
            # Add confidence intervals if requested
            if return_confidence and self.enable_uncertainty_estimation:
                confidence = {}
                ...
                if self.uncertainty_method == 'dropout':
                    ...
                    for outcome in self.outcome_types:
                        ...
                        for _ in range(self.dropout_samples):
                            pred = self.outcome_heads[outcome](hidden_states)
                            samples.append(pred)
                ...
                result['confidence_intervals'] = confidence
