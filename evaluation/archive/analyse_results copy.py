...
from evaluation.clinical_surgical_evaluator import ClinicalSurgicalEvaluator
...
class MetricDebugger:
    ...
    def _debug_map_computation(self, predictions: np.ndarray, ground_truth: np.ndarray, video_id: str):
        ...
        for action_idx in range(min(100, predictions.shape[1])):  # Debug first 10 actions
        ...
        self.logger.info(f"Overall mAP (first 10 actions): {overall_map:.4f}")
...
def main():
    ...
    try:
        ...
        clinical_results = evaluator.evaluate_clinical_performance(predictions, ground_truth)
        report = generate_clinical_evaluation_report(clinical_results, evaluator)
