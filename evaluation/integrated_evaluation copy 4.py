#!/usr/bin/env python3
"""
CORRECTED Integrated Evaluation for Surgical RL Comparison
...
class IntegratedEvaluationFramework:
    """
    CORRECTED evaluation framework with fair comparison approach
    """
    ...
    def evaluate_single_video_comprehensive(self, models: Dict, video_loader: DataLoader) -> Dict:
        ...
        if max_planning_timesteps > 0:
            for method_name, model in models.items():
                ...
                try:
                    planning_results = self._evaluate_planning_capability(
            ...
            for state in batch_states:
                try:
                    if 'AutoregressiveIL' in method_name:
                        ...
                        pred = self._predict_rl_single_step(model, state, use_world_model=False)
    ...
    def _predict_rl_single_step(self, model, state: np.ndarray, use_world_model: bool = False) -> np.ndarray:
        """Single-step prediction for RL models."""
        ...
        # Ensure action_pred is a numpy array
        if not isinstance(action_pred, np.ndarray):
            action_pred = np.array(action_pred)
        
        # Flatten to handle any unexpected dimensions
        ...
        if len(action_pred) == 100:
            ...
            action_value = float(action_pred[0])
            action_idx = int(action_value) % 100
            ...
            if len(action_pred) > 0:
                ...
                action_binary[:copy_length] = action_pred[:copy_length]
            return action_binary
    ...
    def _evaluate_planning_capability(self, model, states: np.ndarray, actions: np.ndarray,
        ...
        for start_idx in start_indices:
            try:
                if 'AutoregressiveIL' in method_name:
                    ...
                    planning_seq = self._generate_direct_video_planning_sequence(
                        model, states[start_idx], horizon
                    )
    ...
    def _generate_world_model_planning_sequence(self, model_dict, initial_state: np.ndarray, 
        ...
        if world_model is None:
            ...
            return self._generate_direct_video_planning_sequence(rl_policy, initial_state, horizon)
    ...
    def _generate_direct_video_planning_sequence(self, model, initial_state: np.ndarray, 
                                               horizon: int) -> np.ndarray:
        """
        Generate planning sequence for direct video RL.
...
def run_integrated_evaluation(experiment_results: Dict, test_data: Dict, 
    ...
    """
    CORRECTED: Run fair integrated evaluation with proper paradigm respect.
    ...
    logger.info("🎯 CORRECTED INTEGRATED EVALUATION")
    logger.info("=" * 50)
    ...
    # Print summary
    logger.info("🎉 CORRECTED EVALUATION COMPLETED!")
    logger.info("=" * 50)
