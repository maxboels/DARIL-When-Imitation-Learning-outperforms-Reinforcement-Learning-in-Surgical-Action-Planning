...
class AutoregressiveILTrainer:
    ...
    def __init__(self, 
        ...
        self.logger.info("🎓 Autoregressive IL Trainer initialized with SHARED METRICS MODULE")
        self.logger.info(f"   ✅ Using utils.evaluation_metrics for consistency")
    ...
    def _evaluate_action_prediction(self, test_loaders: Dict[str, DataLoader]) -> Dict[str, float]:
        """
        🔧 UPDATED: Evaluate action prediction using metrics module.
        """
        ...
        # Flatten for metric calculation (handle sequence dimension)
        if predictions.ndim == 3:  # [batch, seq_len, num_actions]
            # Take last timestep for autoregressive models
            pred_flat = predictions[:, -1, :]  # [batch, num_actions]
            target_flat = targets[:, -1, :]    # [batch, num_actions]
        ...
        metrics = calculate_comprehensive_action_metrics(
            ...
            method_name="AutoregressiveIL_training"
        )
        
        # Log metrics summary
        surgical_metrics.log_metrics_summary(metrics, "Training Validation")
        
        # Extract key metrics for training monitoring
        return {
            'action_mAP': metrics['mAP'],  # Primary metric (present_only)
            'action_mAP_standard': metrics['mAP_standard'],
            'action_mAP_freq_weighted': metrics['mAP_freq_weighted'],
            'action_exact_match': metrics['exact_match'],
            'action_hamming_accuracy': metrics['hamming_accuracy'],
            'action_precision': metrics['precision'],
            'action_recall': metrics['recall'],
            'action_f1': metrics['f1'],
            'action_sparsity': metrics['action_sparsity'],
            'num_actions_present': metrics['num_actions_present']
        }
    
    def _evaluate_single_video(self, test_loader: DataLoader, video_id: str) -> Dict[str, float]:
        """Evaluate model on a single video using metrics module."""
        ...
        # Calculate video-specific metrics using SHARED module
        predictions = np.concatenate(video_predictions, axis=0)
        targets = np.concatenate(video_targets, axis=0)
        
        # Handle sequence dimension
        if predictions.ndim == 3:  # [batch, seq_len, num_actions]
            pred_flat = predictions[:, -1, :]  # Take last timestep
            target_flat = targets[:, -1, :]
        else:
            pred_flat = predictions
            target_flat = targets
        
        # 🔧 SHARED: Use shared metrics module
        metrics = calculate_comprehensive_action_metrics(
            predictions=pred_flat,
            ground_truth=target_flat,
            method_name=f"AutoregressiveIL_video_{video_id}"
        )
        
        return {
            'mAP': metrics['mAP'],
            'mAP_standard': metrics['mAP_standard'],
            'num_frames': len(pred_flat),
            'avg_action_density': np.mean(np.sum(target_flat, axis=1)),
            'action_sparsity': metrics['action_sparsity'],
            'metrics_source': 'metrics_module'  # 🔧 NEW
        }
