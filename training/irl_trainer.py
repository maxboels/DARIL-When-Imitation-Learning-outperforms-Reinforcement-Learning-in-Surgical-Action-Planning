#!/usr/bin/env python3
"""
FIXED: Specific IRL Implementation with proper device handling
Maximum Entropy IRL + Lightweight GAIL with device consistency
"""
...
class MaxEntIRL(nn.Module):
    """
    Maximum Entropy Inverse Reinforcement Learning - FIXED device handling
    """
    ...
    def __init__(self, state_dim: int, action_dim: int, lr: float = 1e-4, device: str = 'cuda'):
        ...
        self.device = device  # FIXED: Store device
    ...
    def learn_reward_from_trajectories(self, expert_trajectories: List[Dict], 
        ...
        """
        Learn reward function using Maximum Entropy IRL - FIXED device handling
        """
        ...
        for traj in expert_trajectories:
            # FIXED: Convert numpy arrays to tensors and move to device
            if isinstance(traj['states'], np.ndarray):
                states = torch.tensor(traj['states'], dtype=torch.float32)
            else:
                states = torch.tensor(traj['states'], dtype=torch.float32)
            
            if isinstance(traj['actions'], np.ndarray):
        ...
        expert_states = torch.cat(expert_states, dim=0).to(self.device)  # FIXED: Move to device
        expert_actions = torch.cat(expert_actions, dim=0).to(self.device)  # FIXED: Move to device
        
        print(f"📊 Expert data shape: states {expert_states.shape}, actions {expert_actions.shape}")
        print(f"🔧 Device: states on {expert_states.device}, actions on {expert_actions.device}, model on {next(self.reward_net.parameters()).device}")
    ...
    def _sample_random_actions(self, num_samples: int) -> torch.Tensor:
        """Sample random actions for negative examples - FIXED device handling"""
        # Sample sparse binary actions (similar to surgical action structure)
        random_actions = torch.rand(num_samples, self.action_dim, device=self.device)  # FIXED: Create on device
...
class LightweightGAIL:
    """
    Lightweight GAIL implementation - FIXED device handling
    """
    
    def __init__(self, il_model, state_dim: int, action_dim: int, lr: float = 1e-4, device: str = 'cuda'):
        ...
        ).to(device)  # FIXED: Move to device
    ...
    def train_gail(self, expert_trajectories: List[Dict], num_epochs: int = 50):
        """
        Train GAIL discriminator and policy adjustment - FIXED device handling
        """
        
        print(f"🎮 Training Lightweight GAIL ({num_epochs} epochs)")
        
        # Prepare expert data - FIXED device handling
        expert_states = []
        expert_actions = []
        
        for traj in expert_trajectories:
            # FIXED: Convert and move to device
            if isinstance(traj['states'], np.ndarray):
                states = torch.tensor(traj['states'], dtype=torch.float32)
            else:
                states = torch.tensor(traj['states'], dtype=torch.float32)
        ...
        print(f"📊 Expert data shape: states {expert_states.shape}, actions {expert_actions.shape}")
        print(f"🔧 Device: states on {expert_states.device}, actions on {expert_actions.device}")
        ...
        for epoch in range(num_epochs):
            ...
            for i in range(0, num_samples, batch_size):
                ...
                with torch.no_grad():
                    # FIXED: Ensure IL model input is on correct device
                    il_input = batch_states.unsqueeze(1)  # Add sequence dimension
                    if hasattr(self.il_model, 'device'):
                        il_input = il_input.to(self.il_model.device)
                    
                    il_predictions = self.il_model.predict_next_action(il_input).squeeze(1)
                    # FIXED: Move IL predictions to GAIL device
                    il_predictions = il_predictions.to(self.device)
            ...
            if epoch % 10 == 0:
                print(f"  Epoch {epoch}: Disc Loss = {epoch_disc_loss/num_batches:.4f}, "
                      f"Policy Loss = {epoch_policy_loss/max(1, num_batches//3):.4f}")
    ...
    def _train_discriminator_step(self, states: torch.Tensor, 
        ...
        """Single discriminator training step - FIXED device handling"""
    ...
    def _train_policy_step(self, states: torch.Tensor, policy_actions: torch.Tensor):
        """Single policy training step - FIXED device handling"""
    ...
    def get_adjusted_prediction(self, state: torch.Tensor) -> torch.Tensor:
        """Get IL prediction with GAIL adjustment - FIXED device handling"""
        
        with torch.no_grad():
            # FIXED: Ensure state is on correct device
...
class ScenarioSpecificIRL:
    """
    Main class combining MaxEnt IRL + Lightweight GAIL - FIXED device handling
    """
    ...
    def __init__(self, il_model, config, logger, device='cuda'):
        ...
        # FIXED: Ensure IL model is on correct device
    ...
    def train_scenario_specific_irl(self, train_data: List[Dict], scenarios_to_train: List[str] = None):
        """Train IRL models for specific scenarios - FIXED device handling"""
            ...
            for scenario_type, frame_indices in video_scenarios.items():
                if scenario_type in scenarios_to_train and frame_indices:
                    ...
                    if isinstance(frame_embeddings, torch.Tensor):
                        frame_embeddings = frame_embeddings.cpu().numpy()
                    if isinstance(actions_binaries, torch.Tensor):
                        actions_binaries = actions_binaries.cpu().numpy()
                    
                    scenario_trajectories = {
                        'video_id': video['video_id'],
                        'states': frame_embeddings[frame_indices],
                        'actions': actions_binaries[frame_indices]
                    }
        ...
        for scenario_type in scenarios_to_train:
            ...
            # Train MaxEnt IRL for reward learning - FIXED device handling
            maxent_irl = MaxEntIRL(
                state_dim=1024,  # Your embedding dimension
                action_dim=100,   # Your action classes
                lr=1e-4,
                device=self.device  # FIXED: Pass device
            ).to(self.device)
            ...
            # Train Lightweight GAIL for policy adjustment - FIXED device handling
    ...
    def predict_with_irl(self, state: torch.Tensor, scenario_type: str = 'standard') -> torch.Tensor:
        """Predict next action using appropriate IRL model - FIXED device handling"""
        ...
        if scenario_type == 'standard' or scenario_type not in self.gail_models:
            with torch.no_grad():
                ...
                if hasattr(self.il_model, 'device'):
                    il_input = il_input.to(self.il_model.device)
                return self.il_model.predict_next_action(il_input).squeeze().to(self.device)
    ...
    def evaluate_irl_vs_il(self, test_data: List[Dict]) -> Dict[str, Any]:
        """Comprehensive evaluation of IRL vs IL - FIXED device handling"""
        ...
        for video in test_data:
            ...
            # FIXED: Handle tensor conversion and device placement
            frame_embeddings = video['frame_embeddings']
            actions_binaries = video['actions_binaries']
            
            # Convert to tensors if numpy
            if isinstance(frame_embeddings, np.ndarray):
                states = torch.tensor(frame_embeddings, dtype=torch.float32).to(self.device)
            else:
                states = frame_embeddings.to(self.device)
            
            if isinstance(actions_binaries, np.ndarray):
                true_actions = torch.tensor(actions_binaries, dtype=torch.float32)
            else:
                true_actions = actions_binaries.cpu()  # Keep on CPU for evaluation
            ...
            for i in range(len(states)):
                ...
                with torch.no_grad():
                    ...
                    if hasattr(self.il_model, 'device'):
                        il_input = il_input.to(self.il_model.device)
                    il_pred = self.il_model.predict_next_action(il_input)
    ...
    def _calculate_map(self, predictions: torch.Tensor, targets: torch.Tensor) -> float:
        """Calculate mean Average Precision - FIXED device handling"""
        try:
            from sklearn.metrics import average_precision_score
            
            # FIXED: Ensure tensors are on CPU for sklearn
            ...
            print(f"Warning: mAP calculation failed ({e}), using accuracy")
            # Fallback to simple accuracy
            predictions = predictions.detach().cpu()
            targets = targets.cpu() if hasattr(targets, 'cpu') else targets
            pred_binary = (predictions > 0.5).float()
            accuracy = (pred_binary == targets).float().mean()
            return accuracy.item()

# Integration function - FIXED device handling
def train_surgical_irl(config, train_data, test_data, logger, il_model):
    """
    Complete IRL training pipeline with FIXED device handling
    """
    
    logger.info("🎯 Training Surgical IRL (FIXED Device Handling)")
    
    # FIXED: Get device from config or detect
    device = config.get('device', 'cuda' if torch.cuda.is_available() else 'cpu')
    logger.info(f"🔧 Using device: {device}")
    
    # Initialize scenario-specific IRL with device
    irl_system = ScenarioSpecificIRL(
        il_model=il_model,
        config=config,
        logger=logger,
        device=device  # FIXED: Pass device explicitly
    )
    ...
    # Log results
    logger.info("🏆 IRL TRAINING RESULTS (FIXED):")
    ...
    return {
        ...
        'technique': 'MaxEnt IRL + Lightweight GAIL (FIXED Device Handling)',
        'device': device
    }

if __name__ == "__main__":
    print("🎯 FIXED SCENARIO-SPECIFIC IRL FOR SURGICAL ACTION PREDICTION")
    print("=" * 60)
    print("✅ FIXED: Proper device handling for CUDA/CPU tensors")
